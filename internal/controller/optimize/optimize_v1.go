...
func (c *ControllerV1) SseGenerate(ctx context.Context, req *v1.OptimizeSseGenerateReq) (
	resp *v1.OptimizeSseGenerateRes, err error) {
	...
	go func() {
		...
		defer func() {
			if r := recover(); r != nil {
				errMsg := fmt.Sprintf("panic in %s, err: %v", "Optimize SseGenerate", r)
		...
		in := &optimizein.OptimizeSseGenerateRequest{}
		err := gconv.Struct(req, in)
		if err != nil {
			return
		}
		err = service.Optimize().SseGenerate(ctx, in, result)
		if err != nil {
			g.Log().Errorf(ctx, "Optimize SseGenerate err: %v", err)
